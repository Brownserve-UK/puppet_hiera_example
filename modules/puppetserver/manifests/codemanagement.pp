...
# @param sources
#  The r10k sources to use
# @param r10k_version
#   The version of r10k to install (defaults to present)
class puppetserver::codemanagement (
  Array[Puppetserver::R10k_source] $sources,
  ...
  cron { 'r10k deploy environment --puppetfile':
    ...
    # r10k can hang especially if it's already running
    # this kills it on the off-chance it's already running. (side-effect is that it will kill any user initiated runs)
